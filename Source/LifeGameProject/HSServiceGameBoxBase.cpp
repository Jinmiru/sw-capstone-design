...
AHSServiceGameBoxBase::AHSServiceGameBoxBase()
{
...
void AHSServiceGameBoxBase::BeginPlay()
{
    Super::BeginPlay();
}

void AHSServiceGameBoxBase::Tick(float DeltaTime)
{
    Super::Tick(DeltaTime);
}

void AHSServiceGameBoxBase::SpawnTrashActors()
{
...
void AHSServiceGameBoxBase::TrashPickedUp()
{
...
void AHSServiceGameBoxBase::CancelGame()
{
...
UHSServiceGameUI* AHSServiceGameBoxBase::GetServiceGameUI()
{
    APlayerController* Controller = UGameplayStatics::GetPlayerController(GetWorld(), 0);
    if (auto* PC = Cast<ALifeGameProjectPlayerController>(Controller))
    {
        UUserWidget* Widget = PC->GetGameUIInstance(EGameUIType::HS_Service);
        return Cast<UHSServiceGameUI>(Widget);
    }
    return nullptr;
}