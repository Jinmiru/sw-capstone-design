...
void AESServiceGameBox::SpawnTrashActors()
{
    ...
    for (int32 i = 0; i < NumTrashToSpawn + 10; ++i)
    {
        ...
        float ExpandedRadius = SpawnRadius * 2.0f; // 기존 반경 2배로 확장
        float Dist = FMath::FRandRange(0.f, ExpandedRadius);
        ...
        FVector End = Origin - FVector(0.f, 0.f, 1000.f); // 아래로 라인트레이스
        ...
        if (bHit)
        {
            FVector SpawnLocation = HitResult.Location + FVector(0.f, 0.f, 10.f);
