...
AESMathGameBox::AESMathGameBox()
{
	...
	CollisionBox->OnComponentBeginOverlap.AddDynamic(this, &AESMathGameBox::OnBeginOverlap);
}

// Called when the game starts or when spawned
void AESMathGameBox::BeginPlay()
{
	Super::BeginPlay();
...
// Called every frame
void AESMathGameBox::Tick(float DeltaTime)
{
	Super::Tick(DeltaTime);

}

void AESMathGameBox::OnBeginOverlap(UPrimitiveComponent* OverlappedComp, AActor* OtherActor, UPrimitiveComponent* OtherComp,
	int32 OtherBoxIndex, bool bFromSweep, const FHitResult& SweepResult)
{
	...
	if (AMyProjectCharacter* Character = Cast<AMyProjectCharacter>(OtherActor))
	{
		...
		if (PlayerController && !PlayerController->IsPlayingMiniGame())
		{
			...
			PlayerController->Client_ShowGameUI(EGameUIType::ES_Math);
		}
	}
}